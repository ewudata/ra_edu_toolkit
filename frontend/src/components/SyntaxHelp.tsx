...
function Example({ database }: { database?: string }) {
  ...
  if (db === 'testdb') {
    return (
      <div className="space-y-1 text-sm font-mono">
        <p className="text-gray-500">-- Names of CS majors</p>
        ...
        <p className="text-gray-500 mt-2">-- Students enrolled in C101 with grades</p>
  ...
  if (db === 'sales') {
    return (
      <div className="space-y-1 text-sm font-mono">
        <p className="text-gray-500">-- Customers and their reps</p>
        ...
        <p className="text-gray-500 mt-2">-- Orders with product descriptions</p>
  ...
  return (
    <div className="space-y-1 text-sm font-mono">
      <p className="text-gray-500">-- Select names of computer science students</p>
      ...
      <p className="text-gray-500 mt-2">-- Find students enrolled in specific courses</p>
...
export default function SyntaxHelp({ database }: Props) {
  ...
  return (
    <div className="space-y-4">
      <h4 className="font-semibold text-gray-800">Relational Algebra Operators</h4>
      <ul className="space-y-1.5 text-sm">
        {ops.map((op) => (
          <li key={op.symbol}>
            <span className="font-bold text-[var(--color-op)]">{op.symbol}</span>{' '}
            <span className="font-semibold">{op.name}</span>:{' '}
            <code className="bg-gray-100 px-1 rounded text-xs">{op.usage}</code>{' '}
            <span className="text-gray-500">— aliases: {op.aliases}</span>
          </li>
        ))}
      </ul>
      <h4 className="font-semibold text-gray-800">Example Queries</h4>
